...
namespace narwhal {
	void KeyboardMovementController::moveInPlaneXZ(GLFWwindow* window, float dt, NarwhalGameObject& gameObject)
	{
		glm::vec3 rotate{ 0 };
		...
		if (glm::dot(rotate, rotate) > std::numeric_limits<float>::epsilon()) { //If any of the keys are pressed (meaning rotation matrix is not 0)
		...
		if (glm::dot(moveDir, moveDir) > std::numeric_limits<float>::epsilon()) { //If any of the keys are pressed (meaning moveDir is not 0)
