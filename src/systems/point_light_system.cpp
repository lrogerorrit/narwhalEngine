...
namespace narwhal {
	...
	PointLightSystem::PointLightSystem(NarwhalDevice& device, VkRenderPass renderPass, VkDescriptorSetLayout globalSetLayout ): narwhalDevice{device}
	{
	...
	void PointLightSystem::createPipeline(VkRenderPass renderPass)
	{
		...
		narwhalPipeline = std::make_unique<NarwhalPipeline>(narwhalDevice, "data/shaders/point_light.vert.spv", "data/shaders/point_light.frag.spv", pipelineConfig);
	...
	void PointLightSystem::update(FrameInfo& frameInfo, GlobalUbo& ubo)	{
		...
		for (auto& kv : frameInfo.gameObjects) {
			...
			assert(lightIndex < MAX_LIGHTS && "Exceded point light max limit!");
	...
	void PointLightSystem::render(FrameInfo& frameInfo){
		...
		VkSpecializationMapEntry entry = { 0, 0, sizeof(int32_t) }; // {constantId, offset, size}
